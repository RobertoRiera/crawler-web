...
class Normalization:
    ...
    def normalize_gray_img(self, grey_img, lex, ley, rex, rey, kind=Kind_wraping.HS):
        ...
        if (angulo < -90) | (angulo > 90):
            cv2.flip(gray_img_rot, 0, gray_img_rot)
        # //Transformamos segun las posiciones de los ojos tras rotar
    ...
    def __obtain_wraping_by_kind(self, kind):
        ...
        if kind == 'HS':
            ...
            c_wraping_img = CWarpingImage([59 + 50 * 2, 65 + 45 * 2],
                                          [16 + 50, 17 + 45, 42 + 50, 17 + 45])
            # tamaño de la cara y posiciones de los ojos para HS
        if kind == 'FACE':
            # //Escalamos segn la distancia ocular y segn la opción de normalización de la ilumicación escogida
